...
#     """Updates the CSV file with KPI entries."""
...
#     try:
#         update_csv_file(kpi_data, append=True)
...
def home():
    return jsonify({"message": "KPI Extraction API is running!"})
...
def process_with_gemini(file_paths):
    ...
    try:
        ...
        try:
            ...
            return {"error": "Failed to parse JSON response", "raw_response": clean_text}
    except Exception as e:
        return {"error": f"Error processing with Gemini: {str(e)}"}
...
@app.route("/extract_kpi/", methods=["POST"])
def extract_kpi():
    """Handles multiple PDF file uploads and extracts combined KPIs."""
    temp_files = []
    try:
...
@app.route("/save_kpis/", methods=["POST"])
def save_kpis():
    """Endpoint to save KPIs to CSV when 'Update Backend CSV' button is pressed."""
    try:
        kpi_data = request.json
        update_csv_file(kpi_data, append=True)
        return jsonify({"message": "KPI data saved successfully."})
    except Exception as e:
        return jsonify({"error": f"Error saving KPI data: {str(e)}"})
